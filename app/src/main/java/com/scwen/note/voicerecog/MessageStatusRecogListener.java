...
//        String message = "识别结束，结果是”" + results[0] + "”";
        ...
        if (speechEndTime > 0) {
            ...
            Log.e("TAG", "；说话结束到识别结束耗时【" + diffTime + "ms】" + currentTime);
...
//        sendStatusMessage(SpeechConstant.CALLBACK_EVENT_ASR_PARTIAL, message);
        if (speechEndTime > 0) {
            ...
            Log.e("TAG", "；说话结束到识别结束耗时【" + diffTime + "ms】");
        }
        speechEndTime = 0;
        sendErrorMessage(message, status,new RecoError(errorCode,subErrorCode,descMessage));
    }
    ...
    public void onAsrVolume(int volumePercent, int volume) {
        ...
        float fixVolPercent = Math.min(volumePercent*3f,100f);

        Message msg = Message.obtain();
        msg.what = IStatus.VOLUME_CHANGE;
        msg.arg1 = status;
        msg.obj = fixVolPercent;
    ...
    private void sendErrorMessage(String message, int what, RecoError recoError) {
        if (handler == null) {
            Log.i(TAG, message);
            return;
        }
